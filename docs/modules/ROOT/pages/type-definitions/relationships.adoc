...
=== ConnectAsDuplicate
By default, when new connections are defined of the same type, between the same two nodes, any existing relationships are updated to have the properties of the new relationship.
However, it is also possible to change this default behaviour so that duplicate relationships are created. This is done by defining `connectAsDuplicate: true` on the `@relationship` directive:
...
type Movie {
    title: String!
    released: Int!
    actors: [Person!]! @relationship(type: "ACTED_IN", properties: "ActedIn", direction: IN, connectAsDuplicate: true)
    director: Person! @relationship(type: "DIRECTED", direction: IN)
}
----

It is also possible to define whether or not individual connections should be created as duplicates, or if they should override existing connections.
See more about this behaviour xref::type-definitions/relationships.adoc#as-duplicate[here].
